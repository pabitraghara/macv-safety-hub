  ...
  const fetchObservations = useCallback(
    async (fetchParams?: ObservationListParams) => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await observationsApi.getObservations(finalParams);
  ...
  const createObservation = useCallback(
    async (observation: CreateObservationRequest) => {
      try {
        setLoading(true);
        setError(null);
        const created = await observationsApi.createObservation(observation);
...
// ─── useAdjacentObservations ──────────────────────────────────────────────────

/**
 * Fetches the previous and next observation codes relative to the given code
 * via a dedicated backend endpoint.
 */
export function useAdjacentObservations(code: string) {
  ...
  useEffect(() => {
    ...
    async function fetchAdjacent() {
      setLoading(true);
      try {
        const result = await observationsApi.getAdjacentCodes(code);
        if (cancelled) return;
        setPrevCode(result.prev_code);
        setNextCode(result.next_code);
      } catch {
        if (!cancelled) {
          setPrevCode(null);
          setNextCode(null);
  ...
  return { prevCode, nextCode, loading };
}
...
export function useObservationByCode(code: string) {
  ...
  const fetchObservation = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      setData(await observationsApi.getObservationByCode(code));
    } catch (err) {
      setError((err as ApiError).message);
      setData(null);
    } finally {
  ...
  return {
    ...
    deleteObservation: () =>
      mutate(() => observationsApi.deleteObservation(code)),
    triageObservation: (triageData: TriageObservationRequest) =>
      mutate(() => observationsApi.triageObservation(code, triageData)),
    setStatus: (status: ObservationStatus, notes?: string) =>
      mutate(() =>
        observationsApi.triageObservation(code, {
