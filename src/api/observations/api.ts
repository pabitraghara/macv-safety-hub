...
export class ObservationsApi {
  ...
  ): Promise<Observation[] | PaginatedResponse<Observation>> {
    return request<Observation[] | PaginatedResponse<Observation>>(
      "GET",
      "/api/v1/observations/",
      undefined,
  ...
  async getAdjacentCodes(
    code: string,
  ): Promise<{ prev_code: string | null; next_code: string | null }> {
    return request<{
      prev_code: string | null;
      next_code: string | null;
    }>("GET", `/api/v1/observations/${code}/adjacent`);
  }
  ...
  /** Aggregated dashboard stats over a trailing window (backend default: 30 days). */
  async getStats(days?: number): Promise<ObservationStats> {
    return request<ObservationStats>(
      "GET",
  ...
  /** Daily counts by violation type over a trailing window (backend default: 30 days). */
  async getDailyByViolationType(
    days?: number,
  ): Promise<DailyViolationTypeEntry[]> {
    const res = await request<{ series: DailyViolationTypeEntry[] }>(
      "GET",
      "/api/v1/observations/stats/daily-by-violation-type",
      undefined,
      days === undefined ? undefined : { params: { days } },
    );
    return res.series;
...
export const observationsApi = new ObservationsApi();
